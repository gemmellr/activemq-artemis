...
public class MQTTProtocolHandler extends ChannelInboundHandlerAdapter {
   ...
   void handleConnect(MqttConnectMessage connect) throws Exception {
      ...
      if (handleLinkStealing() == LinkStealingResult.NEW_LINK_DENIED) {
         ...
         protocolManager.getSessionStateManager().addConnectedClient(session.getConnection().getClientID(), session.getConnection());
      }
   ...
   private LinkStealingResult handleLinkStealing() throws Exception {
      ...
      if (protocolManager.getSessionStateManager().isClientConnected(clientID)) {
         MQTTConnection existingConnection = protocolManager.getSessionStateManager().getConnectedClient(clientID);
